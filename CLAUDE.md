...
- **On-demand revalidate:** yayınla/taslağa-çek/düzenle `revalidatePath` ile `/blog`, `/blog/[slug]` ve anasayfayı **yeniden deploy olmadan** canlı tazeler (Task 9 üretim E2E'de doğrulandı). **DİKKAT:** `sitemap.xml` `force-static` olduğu için `revalidatePath("/sitemap.xml")` onu ÜRETİMDE tazeleMEZ — sitemap yeni/çıkarılan yazıyı ancak sonraki deploy'da yansıtır (çözüm bekliyor; bkz. `src/app/sitemap.ts` yorumu + `.superpowers/sdd/task-9-report.md`).
