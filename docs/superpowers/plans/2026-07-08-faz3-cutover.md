...
- **SEN (opsiyonel):** GoDaddy hosting/statik dosyalar bir süre yedek kalabilir; DNS Vercel'de kaldıkça görünmez.
- **BEN:** Vercel Cron için `purgeOldRequests`'i çağıran, `CRON_SECRET` benzeri başlıkla korunan route handler ekle; Cron'u bu HTTP path'e bağla.
