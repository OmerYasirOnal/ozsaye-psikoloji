...
describe("telegram: selectDraftsToNotify (bildirildi TEKRAR gönderilmez)", () => {
  ...
  it("varsayılan: yalnız 'taslak' (bildirilmiş atlanır)", () => {
    expect(selectDraftsToNotify(drafts, {}).map((x) => x.slug)).toEqual(["a"]);
  });
  it("--yeniden: paylaşılan hariç her şey (bildirildi dahil)", () => {
    expect(selectDraftsToNotify(drafts, { yeniden: true }).map((x) => x.slug)).toEqual(["a", "b", "d"]);
  });
  it("slug filtresi", () => {
...
function mockSpawn({ status = 0, onCall } = {}) {
  const calls = [];
